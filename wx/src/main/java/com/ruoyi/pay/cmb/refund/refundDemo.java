package com.ruoyi.pay.cmb.refund;
...
public class refundDemo {
    ...
    //private static final String url = "https://api.cmburl.cn:8065/polypay/v1.0/mchorders/refund";
    private static final String url = "http://polypay-api.paasuat.cmbchina.cn/api/refund";
    ...
    private static String signMethod() {
        ...
        try {
            ...
            requestTransactionParams.put("notifyUrl", notifyUrl); //交易通知地址
            requestTransactionParams.put("txnAmt", "101"); //交易金额，单位为分（必传）
            requestTransactionParams.put("refundAmt", "11"); //退款金额，单位为分（必传）
            requestTransactionParams.put("origOrderId", "9001004180329103417277"); //原交易的商户订单号，此字段和原交易平台订单号字段至少要上送一个，若两个都上送，则以原交易平台订单号为准
            requestTransactionParams.put("origCmbOrderId", "900119022715203210134445"); //原交易招行订单号，此字段和原交易商户订单号字段至少要上送一个，若两个都上送，则以此字段为准
            ObjectMapper mapper = new ObjectMapper();
