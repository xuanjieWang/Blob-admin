package com.ruoyi.pay.cmb.onlinepay;
 ...
 * 微信统一下单
 */
public class onlinepayDemo {

    //private static final String url = "https://api.cmburl.cn:8065/polypay/v1.0/mchorders/onlinepay";
    private static final String url = "http://polypay-api.paasuat.cmbchina.cn/api/onlinepay";
    ...
    private static String signMethod() {
        ...
        try {
            ...
            requestTransactionParams.put("subAppId", "wx2421b1c4370ec43b"); //子公众帐号ID（必传）
            requestTransactionParams.put("orderId", "1562036392778"); //商户订单号(必传)
            requestTransactionParams.put("tradeType", "APP"); //交易类型 APP/MWEB/JSAPI（必传）
            requestTransactionParams.put("tradeScene", "OFFLINE"); //交易场景
            requestTransactionParams.put("userId", "N002852494");   //收银员(必传)
            requestTransactionParams.put("body", "聚合支付测试"); //商品描述（必传）
            ...
            requestTransactionParams.put("spbillCreateIp", "117.136.40.178"); //终端IP（必传）
            requestTransactionParams.put("sceneInfo", "{\"store_info\" : {\"id\": \"SZTX001\",\"name\": \"腾大餐厅\",\"area_code\": \"440305\",\"address\": \"科技园中一路腾讯大厦\" }}"); //场景信息，H5支付时必传
            ObjectMapper mapper = new ObjectMapper();
